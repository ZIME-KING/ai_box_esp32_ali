...
//  lcd_show_text_string_append(&title_show, "交互启动中 ...");
  ...
  ESP_LOGI(TAG, "conversation_connect done(%d).", ret);
  if (ret != CONV_SUCCESS) {
    return;
  }
...
//  lcd_show_text_string_append(&title_show, "开始交互吧！");
  ...
  ret = conversation_disconnect();
  if (ret != CONV_SUCCESS) {
    ESP_LOGE(TAG, "conversation_disconnect failed(%d).", ret);
  }
  ...
  ret = destroy_conversation();
  if (ret != CONV_SUCCESS) {
    ESP_LOGE(TAG, "destroy_conversation failed(%d).", ret);
  }
