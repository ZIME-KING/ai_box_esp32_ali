...
/* 字库区域占用的总扇区数大小(3个字库+unigbk表+字库信息=3238700
 * 字节,约占791个扇区,一个扇区4K字节) */
...
/* 更新时的提示信息 */
const char *FONT_UPDATE_REMIND_TBL[3] = {
 ...
 * @retval      ESP_OK:表示成功;其他:表示失败
 */
esp_err_t fonts_partition_write(void *buffer, uint32_t offset,
                                uint32_t length) {
  esp_err_t err;
 ...
 * @retval      无
 */
void fonts_progress_show(uint16_t x, uint16_t y, uint8_t size, uint32_t totsize,
                         uint32_t pos, uint32_t color) {
  float prog;
  uint8_t t = 0XFF;
...
static uint8_t fonts_update_fontx(uint16_t x, uint16_t y, uint8_t size,
  ...
  if (rval == 0) {
    ...
    while (res == FR_OK) /* 死循环执行 */
    {
      ...
      fonts_partition_write(tempbuf, offx + flashaddr,
                            bread); /* 从0开始写入bread个数据 */
      offx += bread;
      fonts_progress_show(x, y, size, fftemp->obj.objsize, offx,
                          color); /* 进度显示 */
...
uint8_t fonts_update_font(uint16_t x, uint16_t y, uint8_t size, uint8_t *src,
  ...
  for (i = 0; i < 4; i++) /* 先查找文件UNIGBK,GBK12,GBK16,GBK24 是否正常 */
  {
    ...
    if (res) {
      rval |= 1 << 7; /* 标记打开文件失败 */
      ESP_LOGE(fonts_tag, "Open %s failed", pname);
      break; /* 出错了,直接退出 */
    }
  }
  ...
  if (rval == 0) /* 字库文件都存在. */
  {
    spilcd_show_string(x, y, 240, 320, size, "Erasing sectors... ",
                       color); /* 提示正在擦除扇区 */
    ...
    for (i = 0; i < FONTSECSIZE; i++) /* 先擦除字库区域,提高写入速度 */
    {
      fonts_progress_show(x + 20 * size / 2, y, size, FONTSECSIZE, i,
                          color); /* 进度显示 */
      fonts_partition_read((uint8_t *)buf, ((FONTINFOADDR / 4096) + i) * 4096,
                           4096); /* 读出整个扇区的内容 */
      ...
      if (j != 1024) {
        fonts_partition_erase_sector(((FONTINFOADDR / 4096) + i) *
                                     4096); /* 需要擦除的扇区 */
      }
    }
    ...
    {
      spilcd_show_string(x, y, 240, 320, size,
                         (char *)FONT_UPDATE_REMIND_TBL[i], color);
      strcpy((char *)pname, (char *)src); /* copy src内容到pname */
      strcat((char *)pname, (char *)FONT_GBK_PATH[i]); /* 追加具体文件路径 */
    ...
    /* 全部更新好了 */
    ftinfo.fontok = 0XAA;
    fonts_partition_write((uint8_t *)&ftinfo, FONTINFOADDR,
                          sizeof(ftinfo)); /* 保存字库信息 */
  }
...
uint8_t fonts_init(void) {
  ...
  {
    t++;
    fonts_partition_read((uint8_t *)&ftinfo, FONTINFOADDR,
