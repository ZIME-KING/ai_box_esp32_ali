...
void spilcd_clear(uint16_t color) {
  ...
  uint16_t *buffer =
      heap_caps_malloc(spilcddev.width * sizeof(uint16_t) * 40, MALLOC_CAP_DMA);
  uint16_t color_tmp = ((color & 0x00FF) << 8) |
                       ((color & 0xFF00) >> 8); /* 需要转换一下颜色值 */
  if (NULL == buffer) {
    ...
    for (uint16_t y = 0; y < spilcddev.height; y += 40) {
      esp_lcd_panel_draw_bitmap(panel_handle, 0, y, spilcddev.width, y + 40,
                                buffer);
    }
  }
 ...
 * @retval      无
 */
void spilcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
                 uint16_t color) {
  /* 计算填充区域宽度 */
  ...
  /* 分配内存 */
  uint16_t *buffer =
      heap_caps_malloc(width * sizeof(uint16_t), MALLOC_CAP_INTERNAL);
  uint16_t color_tmp = ((color & 0x00FF) << 8) |
                       ((color & 0xFF00) >> 8); /* 需要转换一下颜色值 */
  ...
  if (NULL == buffer) {
    ...
    for (uint16_t y = 0; y < height; y++) {
      esp_lcd_panel_draw_bitmap(panel_handle, sx, sy + y, ex, sy + y + 1,
                                buffer);
    }
 ...
 * @retval      无
 */
void spilcd_draw_line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                      uint16_t color) {
  uint16_t t;
 ...
 * @retval      无
 */
void spilcd_draw_rectangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
                           uint16_t color) {
 ...
 * @retval      无
 */
void spilcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size,
  ...
  ch_size = ((size / 2) / 8 + (((size / 2) % 8) ? 1 : 0)) *
            size; /* 得到字体一个字符对应点阵集所占的字节数 */

  pcolor =
      heap_caps_malloc(size * (size / 2) * 2, MALLOC_CAP_DMA); /* 申请大小 */
  uint16_t color_tmp = ((color & 0x00FF) << 8) |
                       ((color & 0xFF00) >> 8); /* 需要转换一下颜色值 */
  ...
  ch_offset = chr - ' '; /* 得到偏移后的值（ASCII字库是从空格开始取模，所以-'
                            '就是对应字符的字库） */
  ...
  switch (size) {
    ...
    default:
      ESP_LOGE(TAG, "pcolor %p error size:%d", pcolor, size);
  ...
  for (byte_index = 0; byte_index < ch_size; byte_index++) {
    ...
    for (bit_index = 0; bit_index < 8; bit_index++) /* 一个字节8个点 */
    {
      ...
      // ESP_LOGI("TAG", "pcolor %p op %d", pcolor, pix_index);
      if (pix_index >= size * (size / 2) * 2) {
        ESP_LOGE("TAG", "pcolor %p pix_index is too big:%d", pcolor, pix_index);
      }
      pcolor[pix_index] = colortemp;
      pix_index++;

      if ((size == 24) &&
          (byte_index % 2)) /* 24号字体比较特殊,奇数字节只有四位有效 */
      {
        if (bit_index == 3) {
          break;
        }
  ...
  esp_lcd_panel_draw_bitmap(panel_handle, x, y, x + ch_width, y + ch_height,
                            (uint16_t *)pcolor);
 ...
 * @retval      无
 */
void spilcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len,
  ...
  for (t = 0; t < len; t++) /* 按总显示位数循环 */
  {
    ...
    {
      if (temp == 0) {
        spilcd_show_char(x + (size / 2) * t, y, ' ', size, 0,
                         color); /* 显示空格,占位 */
        continue;                /* 继续下个一位 */
      } else {
        enshow = 1; /* 使能显示 */
    ...
    spilcd_show_char(x + (size / 2) * t, y, temp + '0', size, 0,
                     color); /* 显示字符 */
  }
}
 ...
 * @retval      无
 */
void spilcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len,
  ...
  for (t = 0; t < len; t++) /* 按总显示位数循环 */
  {
    ...
    if (enshow == 0 && t < (len - 1)) /* 没有使能显示,且还有位要显示 */
    {
      if (temp == 0) {
        ...
        {
          spilcd_show_char(x + (size / 2) * t, y, '0', size, mode & 0X01,
                           color); /* 用0占位 */
        } else {
          spilcd_show_char(x + (size / 2) * t, y, ' ', size, mode & 0X01,
                           color); /* 用空格占位 */
        }
        continue;
      } else {
    ...
    spilcd_show_char(x + (size / 2) * t, y, temp + '0', size, mode & 0X01,
                     color);
  }
}
 ...
 * @retval      无
 */
void spilcd_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height,
